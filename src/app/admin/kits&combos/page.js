...
export default function KitsAndCombosPage() {
  ...
  const handleEdit = (item) => {
    ...
    const productsForForm = (activeTab === 'kits' ? item.kit_products : item.combo_products)?.map(includedProduct => ({
          ...
          <>
            ...
            <div className="flex border-b border-gray-200 mb-6">
              <button
                ...
                className={`py-2 px-4 text-sm font-medium ${activeTab === 'kits' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Kits
              </button>
              <button
                onClick={() => setActiveTab('combos')}
                className={`py-2 px-4 text-sm font-medium ${activeTab === 'combos' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Combos
              </button>
            </div>
                    ...
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan="7" className="px-6 py-12 text-center text-gray-500">
                          ...
                          <p className="text-lg font-medium">No {itemLabel}s found</p>
                          <p className="text-sm">Try adding a new {itemLabel.toLowerCase()} or adjusting your search criteria</p>
                        </td>
                      </tr>
                    ) : (
                      filteredItems.map((item) => (
                        <React.Fragment key={item.id}>
                          <tr className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-4 text-sm text-gray-900">
                              <span className="font-medium">{(activeTab === 'kits' ? item.kit_products : item.combo_products)?.length || 0}</span>
                            ...
                            <tr>
                              <td colSpan={7} className="px-0 py-0 bg-gray-50">
                                <div className="px-6 py-4">
                                  <h4 className="text-sm font-semibold text-gray-700 mb-3">Included Products & Variants</h4>
                                  <div className="overflow-x-auto">
                                    {(() => {
                                      const includedProducts = activeTab === 'kits' ? item.kit_products : item.combo_products;
                                      ...
                                      if (includedProducts.some(p => p.product?.variants?.length > 0)) {
                                          if (!fields.includes("quantity")) fields.push("quantity");
                                          if (!fields.includes("unit")) fields.push("unit");
                                          if (!fields.includes("price")) fields.push("price");
                                          if (!fields.includes("stock")) fields.push("stock");
                                      }
                                      ...
                                      return (
                                        <table className="min-w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                                          <thead className="bg-white">
                                            <tr>
                                              <th className="px-4 py-3 text-left font-medium text-gray-700 border-b">Product Name</th>
