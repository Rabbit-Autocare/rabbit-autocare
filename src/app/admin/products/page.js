...
export default function ProductsPage() {
	...
	const handleDelete = async (id) => {
		...
		try {
			...
			if (
				...
				Array.isArray(productToDelete.image) &&
				productToDelete.image.length > 0
			) {
				const { error: storageError } = await supabase.storage
					.from("product-images")
					.remove(productToDelete.image);
	...
	// Helper function to get image URL - Fixed for JSONB array
	const getImageUrl = (imageArray) => {
		...
		if (typeof imageArray === "string") {
			if (imageArray.startsWith("http")) return imageArray;
			return `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/product-images/${imageArray}`;
		}

		return null;
	};

	// Get all image URLs for modal display
	const getAllImageUrls = (imageArray) => {
				...
				{modalProduct && (
					<Modal onClose={() => setModalProduct(null)}>
						<div className="p-6">
							...
							{modalProduct.image && (
								<div className="mb-4">
									{getAllImageUrls(modalProduct.image).length > 0 ? (
										<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
											{getAllImageUrls(modalProduct.image).map(
												(imageUrl, index) => (
													<div
														key={index}
														className="relative w-full h-48 rounded overflow-hidden border border-gray-300"
													>
														<Image
															src={imageUrl}
															alt={`${modalProduct.name} - Image ${index + 1}`}
															fill
