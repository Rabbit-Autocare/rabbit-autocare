...
export default function CartDrawer() {
  const { isCartOpen, closeCart, cartItems, loading, cartCount, mounted } = useCart()
  ...
  return (
    <Drawer isOpen={isCartOpen} onClose={closeCart} position="right" className="w-full sm:w-96 max-w-full">
      {/*
        The key is to create a proper flex container that takes full height
        and allows the middle section to be scrollable
      */}
      <div className="h-[100vh] flex flex-col overflow-hidden bg-white">
        ...
        <motion.div
          ...
          className="flex-shrink-0 p-4 border-b flex items-center bg-white shadow-sm z-10 relative"
        >
          <button
            onClick={closeCart}
        ...
        {/* Fixed banner - also flex-shrink-0 */}
        <motion.div
        ...
        {/*
          Scrollable middle section - flex-1 makes it take remaining space
          min-h-0 is crucial for allowing flex items to shrink below content size
        ...
        */}
          ...
          {loading ? (
            ...
            >
              ...
              {/*
                ...
                This should be at least the height of your footer
              */}
        ...
        {/* Fixed footer - flex-shrink-0 prevents it from shrinking */}
        {cartItems.length > 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3, delay: 0.2 }}
            className="flex-shrink-0 border-t p-4 bg-white shadow-lg z-10 relative"
          >
            <Link
              href="/checkout"
              onClick={closeCart}
