...
export default function CartItem({ item, formatPrice, getVariantDisplayText }) {
      ...
      if (item.combo_id) {
        ...
        if (details) {
          updateItemPrice(item.id, {
            combo_price: details.price,
            combo_price_excluding_gst: details.price / 1.18,
          });
        }
      } else if (item.kit_id) {
        ...
        if (details) {
          updateItemPrice(item.id, {
            kit_price: details.price,
            kit_price_excluding_gst: details.price / 1.18,
          });
        }
      }
    }
  ...
  const includedVariants =
    (isCombo || isKit) && Array.isArray(item.variant) ? item.variant : null;

  const displayName = comboKitDetails?.name || productName;
  const displayImage =
    comboKitDetails?.main_image_url ||
    comboKitDetails?.image_url ||
    productImage;
        ...
        {includedVariants && comboKitDetails ? (
          <div className='mt-2'>
            ...
            <ul className='text-xs text-gray-700 pl-4 list-disc'>
              {(isCombo
                ? comboKitDetails.combo_products
                : comboKitDetails.kit_products
              ).map((v, idx) => (
                <li key={v.variant_id || idx} className='flex items-center gap-2'>
                  {v.product?.main_image_url && (
                    <Image
                      src={v.product.main_image_url}
                      alt={v.product.name}
                      width={24}
                      height={24}
                  ...
                  <span>
                    {v.product?.name || `Product ID: ${v.product_id}`}
                  </span>
          ...
          item.variant && (
            <>
              ...
              {/* Display variant code - stored in variant object */}
              {item.variant.variant_code && (
                <p className='text-gray-400 text-xs mt-0.5'>
                  Code: {item.variant.variant_code}
                </p>
              )}
            </>
          )
