...
export default function MobileNavbar() {
  ...
  const { openCart, cartCount, mounted: useCartMounted, user } = useCart()
  ...
  // Fetch categories - same logic as BottomNavbar
  useEffect(() => {
    if (!mounted) return
    ...
    const fetchCategories = async () => {
      setLoading(true)
      try {
        console.log("Mobile: Attempting to fetch categories...")
        ...
        console.log("Mobile: Transformed categories:", transformedCategories)
  ...
  // Display safe fallback during SSR/hydration
  const displayCartCount = useCartMounted ? cartCount : 0
          ...
          <div className="flex items-center space-x-3">
            ...
            <Link href={user ? "/user" : "/login"}>
              <button className="h-auto w-auto p-0 hover:bg-transparent bg-transparent border-none cursor-pointer transition-colors">
                ...
                <span className="sr-only">{user ? "User account" : "Login"}</span>
              </button>
            </Link>
            ...
            >
              ...
              {useCartMounted && displayCartCount > 0 && (
