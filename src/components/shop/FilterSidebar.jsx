...
const FilterSidebar = ({
  ...
  const shouldShowQuantityFilter = () => {
    return (
      category === "car-interior" || category === "car-exterior" || category === "kits-combos" || category === "all"
    )
  }

  const shouldShowSizeAndColorFilter = () => {
    return category === "microfiber-cloths" || category === "kits-combos" || category === "all"
  }
  ...
  const isCategorySelected = (cat) => {
    ...
    if (cat === "Microfiber Cloths" && category === "microfiber-cloths") return true
  ...
  return (
    <aside className="w-full md:w-64 bg-white p-4 rounded-lg h-fit self-start">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onClearFilters} className="text-xs text-gray-500 hover:text-gray-700">
          Clear all
        </button>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {ratings.map((rating) => (
            <label key={rating} className="flex items-center text-sm cursor-pointer select-none">
              <input
                type="radio"
                name="rating"
      ...
      {shouldShowQuantityFilter() && (
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {sizes.map((size) => (
              <label key={size} className="flex items-center text-sm cursor-pointer select-none">
                <input
                  type="checkbox"
      ...
      {shouldShowSizeAndColorFilter() && (
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {colors.map((color) => (
              <label key={color} className="flex items-center text-sm cursor-pointer select-none">
                <input
                  type="checkbox"
      ...
      <button onClick={onClearFilters} className="w-full bg-black text-white py-2 px-4 rounded font-medium text-sm">
        Apply
      </button>
    </aside>
  )
