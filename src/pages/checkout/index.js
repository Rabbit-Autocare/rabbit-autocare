...
export default function CheckoutPage() {
  ...
  const fetchCartData = async () => {
    const [productRes, comboRes, allProductsRes, allCombosRes] = await Promise.all([
    ...
    if (product) {
      ...
      const ocs = comboCartItems
        .map((item) => {
          ...
          if (c) return { ...c, quantity: item.quantity, comboCartItemId: item.id }
          return null
        })
        .filter(Boolean)
  ...
  const removeComboFromOrder = (id) => {
    ...
    if (comboCartItem) {
      ...
      setComboCartItems((prev) => prev.filter((item) => item.id !== comboCartItem.id))
    }
  }
  ...
  const placeOrder = async () => {
    ...
    const { error } = await supabase
      .from('orders')
      .insert([{ user_id: userId, items, total, status: 'pending', address_id: selectedAddressId }])
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
            ...
            addresses.map((addr) => (
              <label
                ...
                className={`border rounded p-3 block cursor-pointer transition-all ${
                  selectedAddressId === addr.id ? 'border-blue-600 bg-blue-50' : 'border-gray-300'
                }`}
              >
                <input
                  type="radio"
        ...
        <div className="bg-white rounded shadow p-4 space-y-4">
          ...
          {(orderProducts.length === 0 && orderCombos.length === 0) && <p>Your cart is empty.</p>}
            ...
            <div key={item.id} className="flex items-center justify-between border-b py-2">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  min={1}
                  value={item.quantity}
                  onChange={(e) => updateComboQuantity(item.id, parseInt(e.target.value))}
                  className="w-16 border rounded p-1 text-center"
                />
                <button
